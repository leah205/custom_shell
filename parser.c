...
static void get_redirects(Cmd *cmd, struct token **tokens, int tokens_num);
static void get_args(Cmd *cmd, struct token **tokens, int tokens_num);
static void get_job_type(Cmd *cmd, struct token **tokens, int tokens_num);

static Cmd *cmd(struct token **tokens, int token_num);

// static void get_job_type(Expr *cmd, struct token **tokens, int tokens_num);{
//     if(i == tokens_num - 1)
// }
...
void get_redirects(Cmd *cmd, struct token **tokens, int tokens_num)
{
    ...
    while (i < tokens_num && tokens[i]->type == REDIRECT)
    {
        ...
        cmd->redirects[cmd->redirectc].op = op;
        cmd->redirects[cmd->redirectc++].file = file;
    }
}

Cmd *cmd(struct token **tokens, int token_num)
{
    Cmd *cmd;
    ...
    // get_job_type(cmd, tokens, token_num);
    return cmd;
}
...
void get_pipeline(struct token **tokens, int tokens_num, Cmd *tasks[])
{
    ...
    while (j == 0 || (i < tokens_num && tokens[i++]->type == PIPE))
    {
